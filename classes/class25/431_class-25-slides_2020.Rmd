...
library(janitor); library(magrittr); library(broom)
library(patchwork); library(knitr); library(naniar)
...
# Working with the `dm192` example
...
## Distribution of our candidate outcomes (Code)
...
p2 <- ggplot(dm192, aes(x = sqrt(sbp))) +
  geom_histogram(bins = 12, col = "white", 
                 fill = "dodgerblue")
...
## Distribution of our candidate outcomes (Plots)
...
ggplot(m1_aug, aes(x = sbp_old, y = sbp, 
  ...
  guides(col = FALSE) +
  labs(title = "Model m1 with",
       subtitle = "approximate 95% uncertainty intervals") 
```
...
Assumptions (see Course Notes, Section 30)
