...
- In Class 22, we discussed slides 1-47.
- In Class 23, we discussed slides 48-78.
- In Class 24, we discussed the remaining slides, starting at 79.
...
# This is where we stopped after Class 23. We'll restart here in Class 24.
...
- Model `mod_1` has the smallest MAPE (mean APE) and RMSPE and maximum error.
- Model `mod_2` has the smallest median absolute prediction error.
...
- `imod_3` has the best R-square, adjusted R-square and $\sigma$.
- `imod_2` wins on AIC, while `imod_1` has the best BIC.
...
1. In-sample model predictions are about equally accurate for each of the three models. Model `imod_3` has the best R-square, adjusted R-square and $\sigma$, while Model `imod_2` wins on AIC, and Model `imod_1` has the best BIC.
...
Excluding subject S-282, `imod_2` is a little better on all measures.
...
- out-of-sample: distributions of errors are similar. Model 1 has smallest MAPE, RMPSE and maximum error, but all three models are pretty close to each other.

With imputation,

- in-sample: nothing disastrous in residual plots, model 3 has the best R-square, adjusted R-square and $\sigma$, while Model 2 wins AIC, and Model 1 has the best BIC.
- out-of-sample: Model 2 has the smallest MAPE, RMSE, but model 3 has the smallest maximum predictive error. If we drop the one worst-fit case, then model 2 sweeps all three categories.
